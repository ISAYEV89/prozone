...
if ($_POST['delid']) 
{
	$controller=1;
	echo $_POST['delid'];
	$blogquera=$db->prepare('SELECT * FROM blog_cat where u_id=:uid and l_id="1" ');
	...
	{	echo "<br>yeniye:".$u_id." kecid-<br>";
		$blogquer=$db->prepare('SELECT * FROM blog_cat where sub_id=:subid ');
		...
		if ($counts>0) 
		{
			...
			{
				$blogquerda=$db->prepare('DELETE FROM blog_cat where u_id=:uid ');
				...
				{	
					...
					{				
						...
						if (is_file('images/'.$blogquersor['picture']) and is_file('images/'.$blogquersor['icon'])) 
						{
			  			...
			  			$deletecatcont2=$db->prepare('DELETE FROM blog_cat_cont where cat_id=:catid ');
	...
	$deletecatcont=$db->prepare('DELETE FROM blog_cat_cont where cat_id=:catid ');
	...
	if (is_file('images/'.$blogquerasor['picture']) and is_file('images/'.$blogquerasor['icon'])) 
	{
		if (unlink('images/'.$blogquerasor['picture']) and unlink('images/'.$blogquerasor['icon'])) 
	  	{
			$blogquerd=$db->prepare('DELETE FROM blog_cat where u_id=:uid ');
