    ...
    <div class="x_panel">
      ...
      <div class="x_content">   
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
          ...
          <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12">Icon <?php echo pxtostr($staticft['blog_cat_ico']); ?><span class="required">*</span>
            </label>
            <div class="col-md-6 col-sm-6 col-xs-12">
...
if ($_POST['btn']) 
{
  ...
  $lngquer=$db->prepare('SELECT MAX(u_id) as max , MAX(ordering) as maxo FROM blog_cat ');
  $lngquer->execute(); 
  ...
  if ($_FILES['pic']['size']<=900000 && in_array($_FILES['pic']['type'], ['jpg','jpeg','png','image/jpeg','image/jpg','image/png']) && $control=move_uploaded_file($_FILES['pic']['tmp_name'], 'images/'.$unique.'.'.$type)  && $_FILES['ico']['size']<=600000 && in_array($_FILES['ico']['type'], ['jpg','jpeg','png','image/jpeg','image/jpg','image/png']) && $control=move_uploaded_file($_FILES['ico']['tmp_name'], 'images/'.$sunique.'.'.$type2)) 
  {
    ...
    for ($i=0; $i <$lngsay ; $i++) 
    {      
      $lngins=$db->prepare('INSERT INTO blog_cat set l_id=:lng2 , `text`=:txt , ordering=:ord , description=:sn , name=:nm , title=:ti , stitle=:sti , keyword=:keyw , picture=:pc , icon=:ico , u_id=:uid , url_tag=:utg , s_id=:sid , sub_id=:subid ');
