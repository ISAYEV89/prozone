<?php 
$shopquera=$db->prepare('SELECT * FROM shop_cat where u_id=:uid limit 1 ');
$shopquera->execute(array('uid'=>s($_GET['val'])));
$count=$shopquera->rowCount();
$shopquerasor=$shopquera->fetch(PDO::FETCH_ASSOC);
if (!$_GET['val'] or $count<1) 
{
  header('Location:'.$site_url.'shop_cat/list/');
  exit();
}
?>
<?php 
...
$lngquera->execute();
$catquer=$db->prepare('SELECT u_id , name , sub_id FROM shop_cat where l_id="1" ');
$catquer->execute();
$shortname=array();
$orgname=array();
$luid=array();
      ...
      <div class="x_content">   
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
          <div class="w3-container">
            ...
            for($i=0;$i<$lngsay;$i++)
            {
              $shopquers=$db->prepare('SELECT * FROM shop_cat where u_id=:uid and l_id=:lid');
              $shopquers->execute(array('uid'=>s($_GET['val']) , 'lid'=>$luid[$i] ));
              $shopquerssor=$shopquers->fetch(PDO::FETCH_ASSOC);
            ?>
              <div id="<?php echo $orgname[$i]; ?>" class="w3-container city" style="display:none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input id="tagurl<?php echo $shortname[$i]; ?>" value='<?php echo $shopquerssor["url_tag"] ?>'   onchange="check<?php echo $shortname[$i]; ?>();" name="ut<?php echo $shortname[$i]; ?>" type="text" class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="names<?php echo $shortname[$i]; ?>" type="text" value='<?php echo $shopquerssor["name"] ?>'   class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="desc<?php echo $shortname[$i]; ?>" type="text" value='<?php echo $shopquerssor["description"] ?>'   class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="tit<?php echo $shortname[$i]; ?>" type="text" value='<?php echo $shopquerssor["title"] ?>'  class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="stit<?php echo $shortname[$i]; ?>" type="text" value='<?php echo $shopquerssor["stitle"] ?>'  class="js-switch date-picker form-control col-md-7 col-xs-12"  />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="key<?php echo $shortname[$i]; ?>" value='<?php echo $shopquerssor["keyword"] ?>' type="text" class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-9 col-sm-9 col-xs-12">
                    <textarea name="txt<?php echo $shortname[$i]; ?>" ><?php echo $shopquerssor["text"] ?></textarea>
                  </div>
                </div>
              </div>
            <?php
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <select class="form-control" name='subids'>
                <option  value="0" <?php if($shopquerssor["sub_id"]==0){echo 'selected="" ';} ?> >Main</option>
                <?php
                ...
                ?>
                  <option  value="<?php echo $catquerft['u_id']; ?>" <?php if($shopquerasor["sub_id"]==$catquerft['u_id']){echo 'selected="" ';} ?> ><?php echo $catquerft['name'] ?></option>
                <?php 
                }
                ?>
              </select>
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input name="sids"  <?php if(is_null($shopquerasor["s_id"])){echo 'checked=""';} ?> class="date-picker form-control col-md-7 col-xs-12 js-switch" type="checkbox">
            </div>
          </div>
          <div class="form-group">
...
?>
  function  check<?php echo $shortname[$i]; ?>()
  {
    ...
    $.ajax({
      ...
      data: { sc: "tagurlsc", name: x , id: <?php echo s($_GET['val']); ?> },
...
if ($_POST['btn']) 
{
  ...
  for ($i=0; $i <$lngsay ; $i++) 
  {      
    $lngins=$db->prepare('UPDATE shop_cat set l_id=:lng2 , `text`=:txt , description=:sn , name=:nm , title=:ti ,  stitle=:sti , keyword=:keyw , u_id=:uid , url_tag=:utg , s_id=:sid , sub_id=:subid where u_id=:uid and l_id=:lng2');
  ...
  if ($control==1) 
  {
    $db->commit(); 
    echo "string";
    header('Location:'.$site_url.'shop_cat/list/456852/');
    exit();
  }
  else
  {
    $db->rollBack();
    header('Location:'.$site_url.'shop_cat/list/456456/');
    exit();
  }
}
?>
