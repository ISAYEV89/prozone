<?php 
$shop_catquera=$db->prepare('SELECT * FROM shop_cat where u_id=:uid limit 1 ');
$shop_catquera->execute(array('uid'=>s($_GET['val'])));
$count=$shop_catquera->rowCount();
$shop_catquerasor=$shop_catquera->fetch(PDO::FETCH_ASSOC);
        ...
        <form method="POST" action="" id="demo-form2" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">   
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <img style="width: 200px;" src="<?php echo $site_url.'images/'.$shop_catquerasor['picture'] ?>"  class="  col-md-7 col-xs-12" checked />
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <img style="width: 150px;" src="<?php echo $site_url.'images/'.$shop_catquerasor['icon'] ?>"  class="  col-md-7 col-xs-12" checked />
            </div>
          </div>
          <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12">New Picture <?php echo pxtostr($staticft['shop_cat_pic']); ?>
            </label>
            <div class="col-md-6 col-sm-6 col-xs-12">
          ...
          <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12">New Icon <?php echo pxtostr($staticft['shop_cat_ico']); ?>
            </label>
            <div class="col-md-6 col-sm-6 col-xs-12">
...
if ($_POST['btn']) 
{
  ...
  {
    $shop_catins=$db->prepare('UPDATE shop_cat set picture=:pc where u_id=:udi ');
    $shop_catinscon=$shop_catins->execute(array('pc'=>$unique.'.'.$type , 'udi'=>s($_GET['val']) ));
    if ($shop_catinscon) 
    {
      if (is_file('images/'.$shop_catquerasor['picture'])) 
      {
        if (!unlink('images/'.$shop_catquerasor['picture']) ) 
        {
          $controller=0;
        }
      }
  ...
  {
    $shop_catins=$db->prepare('UPDATE shop_cat set icon=:ico where u_id=:udi ');
    $shop_catinscon=$shop_catins->execute(array('ico'=>$sunique.'.'.$type2 , 'udi'=>s($_GET['val']) ));
    if ($shop_catinscon) 
    {
      if (is_file('images/'.$shop_catquerasor['icon'])) 
      {
        if (!unlink('images/'.$shop_catquerasor['icon']) ) 
        {
          $controller=0;
        }
      }
