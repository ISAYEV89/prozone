<?php 
$machinequera=$db->prepare('SELECT * FROM shop_item where u_id=:uid limit 1 ');
...
$catequer=$db->prepare('SELECT cat_id FROM shop_cat_item where item_id="'.s($_GET['val']).'" ');
$catequer->execute();
      ...
      <div class="x_content">   
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">   
          <div class="w3-container">
            ...
            for($i=0;$i<$lngsay;$i++)
            {
              $machinequers=$db->prepare('SELECT * FROM shop_item where u_id=:uid and l_id=:lid');
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input value="<?php echo $machinequerssor['price']; ?>" name="pri" type="number" class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
            </div>
          </div>
          <div class="form-group">
...
if ($_POST['btn']) 
{
  ...
  for ($i=0; $i <$lngsay ; $i++) 
  {
    ...
    { 
      echo 'tttt';
      $lngins=$db->prepare('UPDATE shop_item set ordering=:ord , price=:prc , description=:sn , name=:nm , title=:ti , stitle=:sti , keyword=:keyw , u_id=:uid , l_id=:lng2 , s_id=:sid , `text`=:txt , url_tag=:utg  , picture=:pc where u_id=:uid and l_id=:lng2 ');    
      $lnginscon=$lngins->execute(array('lng2'=>$luid[$i] , 'prc'=>s($_POST['pri']) , 'sn'=>s($_POST['desc'.$shortname[$i]]) , 'nm'=>s($_POST['names'.$shortname[$i]]) , 'ti'=>s($_POST['tit'.$shortname[$i]]) , 'sti'=>s($_POST['stit'.$shortname[$i]]) , 'keyw'=>s($_POST['key'.$shortname[$i]]) , 'txt'=>sck($_POST['txt'.$shortname[$i]]) , 'utg'=>s($_POST['urltag'.$shortname[$i]]) ,  'uid'=>s($_GET['val']) , 'ord'=>$ord , 'sid'=>d(s($_POST['sids'])) , 'pc'=>$unique.'.'.$type   ));
      echo $unique.'.'.$type;
    }
    else
    {
      $lngins=$db->prepare('UPDATE shop_item set ordering=:ord , price=:prc , description=:sn , name=:nm , title=:ti , stitle=:sti , keyword=:keyw , u_id=:uid , l_id=:lng2 , s_id=:sid , `text`=:txt , url_tag=:utg where u_id=:uid and l_id=:lng2 ');
      $lnginscon=$lngins->execute(array('lng2'=>$luid[$i] , 'prc'=>s($_POST['pri']) , 'sn'=>s($_POST['desc'.$shortname[$i]]) , 'nm'=>s($_POST['names'.$shortname[$i]]) , 'ti'=>s($_POST['tit'.$shortname[$i]]) , 'sti'=>s($_POST['stit'.$shortname[$i]]) , 'keyw'=>s($_POST['key'.$shortname[$i]]) , 'txt'=>sck($_POST['txt'.$shortname[$i]]) , 'utg'=>s($_POST['urltag'.$shortname[$i]]) ,  'uid'=>s($_GET['val']) , 'ord'=>$ord , 'sid'=>d(s($_POST['sids']))));
    }
    if (!$lnginscon) 
    {
      $lastcont=0;
  ...
  {
    $lnginss=$db->prepare('INSERT INTO shop_cat_item set cat_id=:caid, cont_id=:coid');
  ...
  {
    $lnginsd=$db->prepare('DELETE FROM shop_cat_item where cat_id=:caid and item_id=:coid');
