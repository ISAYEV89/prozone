        ...
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
              ...
              <div id="<?php echo $orgname[$i]; ?>" class="w3-container city" style="display:none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-9 col-sm-9 col-xs-12">
                    ...
                    <textarea name="txt<?php echo $shortname[$i]; ?>" ><?php echo htmlspecialchars(strval($menuquerssor['text'])) ?></textarea>
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              <input name="sidu"  <?php if(is_null($menuquerasor["network"])){echo 'checked=""';} ?> class="date-picker form-control col-md-7 col-xs-12 js-switch" type="checkbox">
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              <input name="sidm"  <?php if(is_null($menuquerasor["module"])){echo 'checked=""';} ?> class="date-picker form-control col-md-7 col-xs-12 js-switch" type="checkbox">
...

{
  ...
  {      
    ...
    $lngins=$db->prepare('UPDATE menu set type=:typse , link=:links , `text`=:txt , description=:sn , name=:nm , title=:ti , keyword=:keyw , url_tag=:utg , s_id=:sid ,  network=:siu ,  module=:sim , sub_id=:subid  where u_id=:uid and l_id=:lng2 ');

    $lnginscon=$lngins->execute(array('typse'=>s($_POST['typ']) , 'lng2'=>$luid[$i] , 'sn'=>s($_POST['desc'.$shortname[$i]]) , 'links'=>s($_POST['link'.$shortname[$i]]) , 'nm'=>s($_POST['names'.$shortname[$i]]) , 'ti'=>s($_POST['tit'.$shortname[$i]]) , 'keyw'=>s($_POST['key'.$shortname[$i]]) , 'txt'=>sck($_POST['txt'.$shortname[$i]]) , 'utg'=>s($_POST['ut'.$shortname[$i]])  , 'uid'=>$uid , 'sid'=>d(s($_POST['sids'])) , 'sim'=>d(s($_POST['sidm'])) , 'siu'=>d(s($_POST['sidu'])) , 'subid'=>s($_POST['subids'])));
