      ...
      <div class="x_content">   
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input name="cl" type="text" class="js-switch date-picker form-control col-md-7 col-xs-12" checked />
            </div>
          </div>
          <div class="form-group">
...
if ($_POST['btn']) 
{
  ...
  for ($i=0; $i <$lngsay ; $i++) 
  {      
    $lngins=$db->prepare('INSERT INTO menu set class=:cl ,   type=:typse , link=:links , l_id=:lng2 , `text`=:txt , ordering=:ord , description=:sn , name=:nm , title=:ti , keyword=:keyw ,  u_id=:uid , url_tag=:utg , s_id=:sid , sub_id=:subid ');
    $lnginscon=$lngins->execute(array('typse'=>s($_POST['typ']) , 'cl'=>s($_POST['cl']) , 'lng2'=>$luid[$i] , 'sn'=>s($_POST['desc'.$shortname[$i]]) , 'links'=>s($_POST['link'.$shortname[$i]]) , 'nm'=>s($_POST['names'.$shortname[$i]]) , 'ti'=>s($_POST['tit'.$shortname[$i]]) , 'keyw'=>s($_POST['key'.$shortname[$i]]) , 'txt'=>sck($_POST['txt'.$shortname[$i]]) , 'utg'=>s($_POST['ut'.$shortname[$i]])  , 'uid'=>$uid , 'ord'=>$ord , 'sid'=>$s , 'subid'=>s($_POST['subids'])));
    if (!$lnginscon) 
    {
      $control=0;
    }
