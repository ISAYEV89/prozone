<?php 
$blogquera=$db->prepare('SELECT * FROM kateqoriyalar where kat_id=:uid limit 1 ');
      ...
      <div class="x_content">   
        <form method="POST" action="" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
          <div class="w3-container">
            ...
            for($i=0;$i<$lngsay;$i++)
            {
              $blogquers=$db->prepare('SELECT * FROM kateqoriyalar where kat_id=:uid and l_id=:lid');
              ...
              <div id="<?php echo $orgname[$i]; ?>" class="w3-container city" style="display:none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="nm<?php echo $shortname[$i]; ?>" type="text" value='<?php echo $blogquerssor["name"] ?>'  class="js-switch date-picker form-control col-md-7 col-xs-12"  />
                  </div>
                </div>
              </div>
            <?php
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input <?php if($blogquerasor["all_country"]==1){echo 'checked=""';} ?>  name="allc" type="checkbox" class="js-switch date-picker form-control col-md-7 col-xs-12"  />
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input name="sids"  <?php if($blogquerasor["s_id"]==0){echo 'checked=""';} ?> class="date-picker form-control col-md-7 col-xs-12 js-switch" type="checkbox">
            </div>
          </div>
          <div class="form-group">
...
if ($_POST['btn']) 
{
  ...
  for ($i=0; $i <$lngsay ; $i++) 
  {      
    $lngins=$db->prepare('UPDATE kateqoriyalar set all_country=:alc , name=:nm ,  s_id=:sid where kat_id=:uid and l_id=:lng2');
    $lnginscon=$lngins->execute(array('lng2'=>$luid[$i] , 'alc'=>d1($_POST['allc']) , 'nm'=>s($_POST['nm'.$shortname[$i]]) , 'uid'=>s($_GET['val']) , 'sid'=>d($_POST['sids']) ));
    if (!$lnginscon) 
    {
      $control=0;
    }
