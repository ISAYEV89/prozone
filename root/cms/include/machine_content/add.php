...
if ($_POST['btn']) 
{
  $lngquer=$db->prepare('SELECT MAX(u_id) as max , MAX(ordering) as maxo FROM texnika_content ');
  $lngquer->execute(); 
  ...
  if ($_FILES['pic']['size']<=600000 && in_array($_FILES['pic']['type'], ['jpg','jpeg','png','image/jpeg','image/jpg','image/png']) && $control=move_uploaded_file($_FILES['pic']['tmp_name'], 'images/'.$unique.'.'.$type)) 
  {
    ...
    for ($i=0; $i <$lngsay ; $i++) 
    {
      $lngins=$db->prepare('INSERT INTO texnika_content set ordering=:ord , description=:sn , name=:nm , title=:ti , stitle=:sti , keyword=:keyw , picture=:pc , u_id=:uid , l_id=:lng2 , s_id=:sid , `text`=:txt , url_tag=:utg');
      $lnginscon=$lngins->execute(array('lng2'=>$luid[$i] , 'sn'=>s($_POST['desc'.$shortname[$i]]) , 'nm'=>s($_POST['names'.$shortname[$i]]) , 'ti'=>s($_POST['tit'.$shortname[$i]]) ,  'sti'=>s($_POST['stit'.$shortname[$i]]) ,  'keyw'=>s($_POST['key'.$shortname[$i]]) , 'txt'=>sck($_POST['txt'.$shortname[$i]]) , 'utg'=>s($_POST['urltag'.$shortname[$i]]) , 'pc'=>$unique.'.'.$type , 'uid'=>$uid , 'ord'=>$ord , 'sid'=>NULL ));
      if (!$lnginscon) 
      {
        echo '_________5___'.$lastcont=0;
      }
    }
    for ($i=0; $i <$catsay ; $i++) 
    { 
      $lnginsa=$db->prepare('INSERT INTO texnika_cat_cont set cat_id=:caid, cont_id=:coid ');
      ...
      { echo "str";
        ...
        { 
          ...
          { echo "string";
            ...
            echo 'INSERT INTO texnika_img set dir=:dire , ordering=:ord , t_id=:tid , l_id=:lid , u_id=:uid ';
            $lnginsa2con=$lnginsa2->execute(array( 'dire'=>$tunique.'.'.$ttypes , 'ord'=>$t , 'uid'=>$tuid , 'lid'=>$luid[$i] , 'tid'=>$uid));
            if (!$lnginsa2con) 
            {
