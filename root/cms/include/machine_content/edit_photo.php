...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Edit machine content's photos </h2><a style="float: right;" class="btn btn-success" href="<?php echo $site_url.'machine_content/add_photo/'.s($_GET['val']).'/' ?>"><i class="fa fa-plus"></i> Əlavə et </a>
          ...
          <div class="w3-container">
            ...
            ?>
              <div id="<?php echo $orgname[$i]; ?>" class="w3-container city" style="display:none">
                ...
                <?php 
                $texnika_contentqueras=$db->prepare('SELECT `bi`.`u_id` , `bi`.`id` , `bi`.`t_id` , `bi`.`name` , `bi`.`alt` , `bi`.`ordering` , `bi`.`dir` FROM texnika_content bc , texnika_img bi where `bc`.`u_id`=:uid and `bc`.`l_id`="1" and `bi`.`t_id`=`bc`.`u_id` and `bi`.`l_id`="'.$luid[$i].'" ');
                $texnika_contentqueras->execute(array('uid'=>s($_GET['val'])));
                $counts=$texnika_contentqueras->rowCount();
                $num=0;
                while($texnika_imgsor=$texnika_contentqueras->fetch(PDO::FETCH_ASSOC))
                {
                $num++;
                $dirarr[$texnika_imgsor['u_id']]=$texnika_imgsor['dir'];
                if (!in_array( $texnika_imgsor['u_id'] ,$uidarr )) 
                {
                  array_push($uidarr , $texnika_imgsor['u_id']);
                }
                ?>
                  <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <img style="width: 150px;" src="<?php echo $site_url.'images/'.$texnika_imgsor['dir'] ?>"  class="col-md-7 col-xs-12"  />
                      <?php
                      if ($i==0) 
                      {
                      ?>
                        <input value="<?php echo $texnika_imgsor['u_id'] ?>" id="<?php echo $texnika_imgsor['u_id']; ?>" name="del[]"   type="checkbox" />
                        <label style="color:red; width: 50px;" class="fa fa-trash fa-3x" for="<?php echo $texnika_imgsor['u_id']; ?>"></label>
                      <?php
                      } 
                      ?>
                    </div>
                  ...
                  ?>
                    <div class="form-group">
                      ...
                      <div class="col-lg-1 col-md-1 col-sm-1 col-xs-3">
                        <input value="<?php echo $texnika_imgsor['ordering'] ?>" name="ord<?php echo $texnika_imgsor['u_id']; ?>" type="number" class=" date-picker form-control col-md-7 col-xs-12" checked />
                      </div>
                    </div>
                  <?php
                  } 
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <input value="<?php echo $texnika_imgsor['name'] ?>" name="names<?php echo $shortname[$i].$texnika_imgsor['u_id']; ?>" type="text" class=" date-picker form-control col-md-7 col-xs-12" checked />
                    </div>
                  </div>
                  <div class="form-group" style="    border-bottom: 1px dashed #000;">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <input value="<?php echo $texnika_imgsor['alt'] ?>" name="alt<?php echo $shortname[$i].$texnika_imgsor['u_id']; ?>" type="text" class=" date-picker form-control col-md-7 col-xs-12" checked />
                    </div>
                  </div> 
                <?php
                }
...
if ($_POST['btn']) 
{
  ...
  for($i=0;$i<$deleteq;$i++)
  {
    ...
    $delete=$db->prepare("DELETE from texnika_img where u_id=:uids");
  ...
  for($i=0;$i<$updateq;$i++)
  {
    if(!in_array($uidarr[$i], $src))
    {
      for($t=0;$t<$lngsay;$t++)
      {
        $update=$db->prepare("UPDATE texnika_img set ordering=:ords , alt=:alts , name=:nms  where u_id=:uids and l_id=:lids");
