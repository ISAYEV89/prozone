              ...
              <div id="<?php echo $orgname[$i]; ?>" class="w3-container city" style="display:none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <input name="sl<?php echo $shortname[$i]; ?>" type="text" class="js-switch date-picker form-control col-md-7 col-xs-12" />
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-9 col-sm-9 col-xs-12">
                    <textarea class="col-md-8 col-lg-8 col-sm-8 col-xs-12" rows="9"  name="cvb<?php echo $shortname[$i]; ?>" ></textarea>
                  </div>
                </div>
              </div>
            <?php
...
if ($_POST['btn']) 
{
  ...
  $ouid=$lngquersor['maxo']+1;
  $control=1;

  $db->begintransaction();
  for ($i=0; $i <$lngsay ; $i++) 
  {        
    $lngins=$db->prepare('INSERT INTO faq set  l_id=:lng2 , `cavab`=:csvb ,  u_id=:uid , sual=:sul , ordering=:ord ');
    $lnginscon=$lngins->execute(array( 'ord'=>$ouid  , 'lng2'=>$luid[$i] ,'csvb'=>sck($_POST['cvb'.$shortname[$i]]) ,'uid'=>$uid ,'sul'=>s($_POST['sl'.$shortname[$i]])));
    if (!$lnginscon) 
    {
      echo $control=0;
    }
