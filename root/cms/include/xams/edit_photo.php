<?php 
$xamsquera=$db->prepare('SELECT * FROM xams where u_id=:uid limit 1 ');
        ...
        <form method="POST" action="" id="demo-form2" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">   
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <img style="width: 150px;" src="<?php echo $site_url.'images/'.$xamsquerasor['icon'] ?>"  class="  col-md-7 col-xs-12" checked />
            </div>
          </div>
          <div class="form-group">
...
if ($_POST['btn']) 
{
  ...
  {
    $xamsins=$db->prepare('UPDATE xams set icon=:ico where u_id=:udi ');
    $xamsinscon=$xamsins->execute(array('ico'=>$sunique.'.'.$type2 , 'udi'=>s($_GET['val']) ));
    if ($xamsinscon) 
    {
      if (is_file('images/'.$xamsquerasor['icon'])) 
      {
        if (!unlink('images/'.$xamsquerasor['icon'])) 
        {
          $controller=0;
        }
      }
