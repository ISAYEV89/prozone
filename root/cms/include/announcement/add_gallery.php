...
$query = $db->prepare("SELECT * FROM announcement_content_gallery where blog_id = ? ORDER BY ordering ASC");
...
?>
    ...
    <a href="<?php echo $site_url.'announcement/delete_gallery/'.$image['id'].'/' ?>"><i class="fa fa-trash fa-2x text-danger"></i></a>
    <form method="post" action="<?php echo $site_url.'announcement/update_gallery/' ?>" style="display:flex;" >
        ...
        foreach($_FILES['files']['name'] as $key=>$val){
            ...
            if(in_array($extension, $allowTypes)){
                ...
                if(move_uploaded_file($_FILES["files"]["tmp_name"][$key], $targetFilePath)){
                    ...
                    // Image db insert sql
                    if($key != intval(count($_POST['order'])) - 1 ){
                        $lngins=$db->prepare('INSERT INTO announcement_content_gallery set blog_id=:id ,ordering=:ordering,image=:image ');
