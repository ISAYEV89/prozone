...
$pcurr=$_SESSION['countrycurrency']; //we'll use it in javascript to calculate the price in different format
	...
	print_r($_SESSION);
	echo'</pre>'; */
if($_SERVER["HTTP_REFERER"]!=$site_url.$lng.'/shop/shippay/' or @!$_SESSION['id'])// check refering page
{
	?>
...
text-shadow: 0 1px 1px rgba(0, 0, 0, 0.15);  cursor:pointer;  font-size: 24px;    font-weight: 300;    border: none;    width: 270px;    color: #fff; padding:7px;  line-height: 30px; margin:0px auto;" id="backBtn" onclick="location.replace('<?PHP echo $site_url.$lng.'/shop/1/'; ?>');">
...
else
{
	...
	$a->execute(ARRAY('lid'=>$lng1, 'olke' => $_SESSION['country']));
				...
				<tbody>
					<tr class="line_info_bot line_info_bot_302 even active_row active_row_b">
						<td colspan="3" class="order_data">
							...
							<form action="" id="registr_form" method="post" onsubmit="return validateme()">
							<table>
								<thead>
								<tr>
								...
								<tbody >
									<tr class="odd">
										<td  style="text-align:right; padding-right:15px;"><input type="radio" id="lbradio" name="payoption" value="1"></td>
										<td>Life bank: <?PHP echo $_SESSION['lb']?> <span id="lbcurr"></span></td>
									</tr>
									<tr>
										<td  style="text-align:right; padding-right:15px;"><input type="radio" id="cbradio" name="payoption" value="2"></td>
										<td>Commision bank: <?PHP echo $_SESSION['cb']?> <span id="cbcurr"></span></td>
									</tr>
									<tr class="odd">
