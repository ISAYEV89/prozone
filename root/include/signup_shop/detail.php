...
<?PHP
	...
	// lazim olan permissionlara baxiriq**********************************
	if(@$_SESSION['signup_referer_id'] && @$_SESSION['signup_country_id'])
	{		
	}
	else
	{		
...
<div class="middle">
	...
	text-shadow: 0 1px 1px rgba(0, 0, 0, 0.15);  cursor:pointer;  font-size: 24px;    font-weight: 300;    border: none;    width: 270px;    color: #fff; padding:7px;  line-height: 30px; margin:0px auto;" id="backBtn"  onclick="location.replace('<?PHP echo $site_url.$lng.'/signup_shop/'; ?>');">
				...
				<div class="block_products_sidebar">
					...
					?>
						<div class="col col_<?PHP echo $record; ?>">
							<div class="image"><a href="<?PHP echo $site_url.$lng.'/signup_shop/'.$lng1cek['kat_id'].'/'; ?>"><img src="<?PHP echo $site_url; ?>cms/images/<?PHP echo  $lng1cek['picture']; ?>" alt="<?PHP echo  $lng1cek['name']; ?>"></a></div>
							<div class="title"><div  class="field_content" ><a href="<?PHP echo $site_url.$lng.'/signup_shop/'.$lng1cek['kat_id'].'/'; ?>"><?PHP echo  $lng1cek['name']; ?></a></div></div>
						</div>
					<?PHP
					}  
					?>
	...
	$lng01sor->execute(array( 'olke'=>$_SESSION['signup_country_id'], 'lang'=>$lng1, 'uid'=>s($_GET['cname']) )); 
	while ($b=$lng01sor->fetch(PDO::FETCH_ASSOC)) 
	{		
		$proid=$b['m_u_id'];
		if($products[$proid])
	...
	if(@$products[$proid]['countryid']) 
	{
		$gf='mop_id=:country';
		$lng01sor=$db->prepare('SELECT md.*, (unix_timestamp(end)-unix_timestamp(now()))/1000 ferq FROM mehsul_discount md where  m_u_id =:uid and s_id=1 and shop_type=:shop_type and '.$gf);
		$lng01sor->execute(array( 'country'=>$_SESSION['signup_country_id'], 'uid'=>s($_GET['cname']), 'shop_type'=>$shop_type ));
	}
	else
	{
		$gf='mop_id=0';
		$lng01sor=$db->prepare('SELECT md.*, (unix_timestamp(end)-unix_timestamp(now()))/1000 ferq FROM mehsul_discount md where  m_u_id =:uid and s_id=1 and shop_type=:shop_type and '.$gf);
		$lng01sor->execute(array( 'uid'=>s($_GET['cname']), 'shop_type'=>$shop_type ));
	}
	...
	foreach ($products as $key => $val) 
	{
		...
		else //yoxdursa qiymetler konvert olunmalidir
		{
			...
			{				
				...
				$products[$key][$shname]=currconverter($db,$products[$key][$shname],$t1,$t2);				
	...
	?>
			<div class="page_content node_goods">
				...
				{
                ?>
					<div class="node_field available"  style="padding-left: 20px !important; background-size: 15px;">
				...
				?>
					<div class="node_field">
						<span class="field_content"><i class="fa fa-times" style="color:red; margin-right: 4px;"></i><?php echo $shopdetail['notavailable'][$lng] ?></span>
					</div>
				<?PHP
				} 
				...
				?>
					...
					<div class="node_field article">
						<span class="label"><img src="<?PHP echo $site_url?>user/images/level_icon.png" style="width: 17px;
    margin-right: 5px; vertical-align: middle;"><?PHP echo $shopdetail['regtype'][$lng] ?>:</span>
				...
				<div class="node_main_wrap clearfix smcs" >
					...
					<?PHP
						...
						if(@$products[$prodid]['discount'])
						{
							...
							echo'<div class="old_price" ">'.$products[$prodid]['price_2'].' '.$short_name.' </div><div class="price">'.$products[$prodid]['disc_price'].' '.$short_name.'<span></span></div>';
						}
						else
						{
							$price_real=$products[$prodid][$tt];
							echo'<div class="price">'.$products[$prodid]['price_2'].' '.$short_name.'<span></span></div>';						
						}
					?>
						<div class="good_basket clearfix">
							...
							<div class="good_basket_add">
								...
								data-ship="<?PHP echo $products[$prodid]['shipping_2'] ;?>"
				...
				<div class="block_goods similar_goods">
					...
					foreach($products as $keyp=>$valuep)
					{
						...
						echo'<div class="col col_'.$disad.'">
							...
							echo '<a href="'.$site_url.$lng.'/signup_shop/1/'.$products[$keyp]['u_id'].'/">';
						...
						echo'</div>
							<div class="title"><a href="'.$site_url.$lng.'/signup_shop/1/'.$products[$keyp]['u_id'].'/">'.$products[$keyp]['name'].'</a></div>
